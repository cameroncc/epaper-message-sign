...
class SnoopyComicAnimation:
    ...
    def draw_snoopy_head(self, draw, x, y, state):
        ...
        nose_y = y + 1
        draw.ellipse(
            [nose_x, nose_y, nose_x + nose_length, nose_y + nose_width], fill=0
        )
    ...
    def draw_snoopy_body(self, draw, x, y, state):
        ...
        collar_y = y + 3
        draw.ellipse(
            [x - body_width // 2 + 1, collar_y, x + body_width // 2 - 1, collar_y + 4],
            fill=0,
            outline=0,
            width=1,
        )
        ...
        if state == "dancing":
            ...
            # Happy arms (raised)
            draw.line(
                [x - body_width // 2, arm_y, x - body_width // 2 - 5, arm_y - 8],
                fill=0,
                width=2,
            )
            draw.line(
            ...
            # Normal arms (down)
            draw.line(
                [x - body_width // 2, arm_y, x - body_width // 2 - 4, arm_y + 6],
                fill=0,
                width=2,
            )
            draw.line(
        ...
        foot_size = 4
        draw.ellipse(
            [x - 6, leg_y + leg_height - 2, x - 2, leg_y + leg_height + 2], fill=0
        )
        draw.ellipse(
            [x + 2, leg_y + leg_height - 2, x + 6, leg_y + leg_height + 2], fill=0
        )
            ...
            if size > 1:
                draw.ellipse(
                    [
                        bubble_x + offset_x,
                        bubble_y + offset_y,
                        bubble_x + offset_x + size,
                        bubble_y + offset_y + size,
                    ],
                    fill=255,
                    outline=0,
                    width=1,
    ...
    def draw_speech_bubble(self, draw, x, y, text):
        ...
        draw.polygon(
            [
                (bubble_x + 3, bubble_y + bubble_height),
    ...
    def draw_ui_elements(self, draw):
        ...
        draw.text(
            (2, self.height - 30), "Touch Bottom: Sleep/Wake", font=font_small, fill=0
        )
        draw.text(
            (2, self.height // 2 - 3), "Touch Middle: Happy", font=font_small, fill=0
        )
        ...
        draw.line(
            [(0, self.height // 3), (self.width, self.height // 3)], fill=0, width=1
        )
        draw.line(
            [(0, 2 * self.height // 3), (self.width, 2 * self.height // 3)],
            fill=0,
            width=1,
        )
        ...
        try:
            ...
            while True:
                ...
                if (
                    ...
                    or self.GT_Old.Y[0] != self.GT_Dev.Y[0]
                    or self.GT_Old.S[0] != self.GT_Dev.S[0]
                ):
