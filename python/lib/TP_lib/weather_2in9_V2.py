...
font20_Roboto_Regular = ImageFont.truetype(
    os.path.join(fontdir, "Roboto-Regular.ttf"), 20
)
...
UNITS = "imperial"
CSV_OPTION = (
    False  # if csv_option == True, a weather data will be appended to 'record.csv'
)
...
def get_weather_png():
    ...
    while error == None:
        ...
        if response.status_code == 200:
            ...
            string_temp_current = format(temp_current, ".0f") + "\N{DEGREE SIGN}F"
            string_temp_current_C = (
                format((temp_current - 32) / 1.8, ".0f") + "\N{DEGREE SIGN}C"
            )
            string_temp = string_temp_current + " / " + string_temp_current_C
            string_feels_like = (
                "Feels like: " + format(feels_like, ".0f") + "\N{DEGREE SIGN}F"
            )
            ...
            string_temp_max = "High: " + format(temp_max, ">.0f") + "\N{DEGREE SIGN}F"
            string_temp_min = "Low:  " + format(temp_min, ">.0f") + "\N{DEGREE SIGN}F"
    ...
    while font_report.getsize(string_report)[0] > 120:
        font_report_size -= 2
        font_report = ImageFont.truetype(
            os.path.join(fontdir, "Roboto-Bold.ttf"), font_report_size
        )
    ...
    draw.text(
        (295 - 11 - font20_Roboto_Regular.getsize("UPDATED")[0], 7),
        "UPDATED",
        font=font20_Roboto_Regular,
        fill=black,
        align="right",
    )
    current_time = datetime.now().strftime("%H:%M")
    draw.text(
        (295 - 11 - font34_Roboto_Black.getsize(current_time)[0], 25),
        current_time,
        font=font34_Roboto_Black,
        fill=black,
        align="right",
    )
    ...
    draw.text(
        (295 - 11 - font18_Roboto_Bold.getsize(string_location)[0], 72),
        string_location,
        font=font18_Roboto_Bold,
        fill=white,
        align="right",
    )
