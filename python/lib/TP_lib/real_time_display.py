...
def get_joke():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            return (
                data["setup"][:50] + "..." if len(data["setup"]) > 50 else data["setup"]
            )
        return "Error"
    except:
        return "Error"
...
def create_display_image():
    ...
    # Draw crypto price
    draw.text(
        (10, 80), f"Bitcoin: {current_data['crypto_price']}", font=font16, fill=black
    )

    # Draw stock price
    draw.text(
        (10, 100), f"AAPL Stock: {current_data['stock_price']}", font=font16, fill=black
    )

    # Draw weather
    draw.text(
        (10, 120),
        f"Weather: {current_data['weather_temp']} - {current_data['weather_desc']}",
        font=font16,
        fill=black,
    )
    ...
    # Draw IP location
    draw.text(
        (10, 140), f"Location: {current_data['ip_info']}", font=font16, fill=black
    )
    ...
    draw.text(
        (10, 200),
        f"Last Update: {current_data['last_update']}",
        font=font12,
        fill=black,
    )
